...
> **이름에 대해** — 이 스킬의 정식 명칭은 **`mbo-천상`**이다. `/mbo`는 그 영구 별칭으로, 설치 후 두 명령 모두 같은 스킬을 부른다. `mbo-천상`은 원래 PO의 운영 환경에서 여러 스킬을 통솔하는 최상위 메타 스킬의 이름이며, 이 저장소(`mbo-skill`)는 그 **핵심인 목표·승인·결과 관리 틀만 자기완결 형태로 공개**한 것이다 — 확장판 없이 이 파일 하나로 단독 동작한다.

---

## The Story

어느 날, PO(인간, 지휘자)가 결심했다.
...
- **합의된 목표는 끝까지 추구** — 통과시키려 기준을 낮추지 않고, 미달이면 정직하게 보고. 과정은 자유, 결과는 책임.
...
1. **합의된 목표는 끝까지 추구** — 통과시키려 기준을 낮추지 않고, 미달이면 정직 보고
...
|------|--------|--------|----------|----------|:----:|
| ...  | ...    | ...    | ...      | ...      | 필수/비핵심 |
(측정 조건 = 통계 기준(평균/p95)·표본·기간·환경 · 핵심 = 부분달성 판정 근거)
...
|----------|----------|----------|:----:|
| ...      | ...      | ...      | 필수/비핵심 |
- **자기검증 금지** — 작성자가 아닌 별도 검증자가 원자료에 직접 접근해 판정
- KPI(수치 측정)와 구분 — 이 기준은 통과/탈락(O/X)을 가른다
