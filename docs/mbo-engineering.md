...
다섯은 그대로 살아 있다. 다만 더는 제각각 흩어진 테크닉이 아니라, '무엇을 향해 가는가'라는 한 축 아래 정렬된 수단이다. 어떤 수단도 스스로 '무엇이 잘된 것인지'를 정의하지 못한다. 합격선은 목적에서만 나온다. 목적이 서야 수단의 자리가 정해진다 — MBO 엔지니어링이 정점인 이유다.

## 작동은 세 단계다

- **목표 정의** — 한 번에 한 질문씩 캐묻는 구조화 문답(기준 20문, 일찍 또렷해지면 단축)으로 막연한 요구를 좁히고, AS-IS를 조사한 뒤, 목표·KPI·산출물·완료검증 기준을 담은 목표서를 만들어 PO 승인을 받는다.
- **실행** — 목표만 합의되면 방법은 AI 자율에 맡긴다.
- **결과 보고** — "했다"가 아니라 "달성/미달성"으로 보고한다. KPI를 실측해 검증 기준과 대조하고, 미달성이면 사유와 후속을 붙인다.

예컨대 목표서 한 줄은 이렇게 적힌다 — 목표 '로그인 API를 화면에 연결한다' · KPI '로그인 성공률 99%, p95 300ms' · 완료·검증 '실제 화면에서 버튼 클릭 → 200 응답을, 만든 사람이 아닌 별도 검증자가 확인'. 추상이 아니라 이렇게 한 줄씩 못 박아 합의한다.

![MBO 스킬 — 관계도 + 흐름도](../assets/mbo-skill-diagram.png)

철학은 단순하다. **과정은 자율, 목표는 합의이자 필수.** 완료기준은 "파일이 있다"가 아니라 "실제로 호출·실행된다". 그리고 자기검증은 금지한다 — 합격 판정은 만든 주체가 아니라 별도 검증자(독립 검증 에이전트 또는 사람 PO)가 내린다. 처음의 빈 껍데기 62개는 바로 이 세 가지가 없어서 나온 결과였다.
